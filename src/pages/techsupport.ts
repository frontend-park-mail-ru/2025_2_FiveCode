...
export async function renderTechSupportPage(): Promise<void> {
  document.body.classList.add("tech-support-body");
  const app = document.getElementById("app")!;
  app.innerHTML = "";

  const page = document.createElement("div");
  page.className = "page tech-support-page-wrapper";
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (Object.keys(errors).length > 0) {
      if (errors.title) {
        titleErrorEl.textContent = errors.title;
        titleErrorEl.classList.add("error-message--visible");
      }
      if (errors.description) {
        descriptionErrorEl.textContent = errors.description;
